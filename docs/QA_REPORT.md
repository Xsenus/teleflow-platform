...
Упало:                       0
Функциональных модулей:      36
Statements:                  17 997
Missed statements:           3 020
Statement coverage:          83,22%
Обязательный gate:           80%
Полный прогон:               590,8 секунды при высокой фоновой нагрузке
Coverage-прогон:             444,3 секунды, два завершённых набора 305 + 26
```

Финальный полный запуск сначала дал 328 passed, 2 ожидаемых skip и единственное падение
проверки уже устаревшего после правки `qa_release.sh` файла `MANIFEST.sha256`. Manifest был
перегенерирован, а тот же regression-тест повторён успешно; продуктовых test failures не было.
Coverage объединён из двух завершённых непересекающихся наборов 305 + 26 тестов. Данные
монолитного запуска, остановленного внешним timeout на загруженной машине, не учитывались.
...
802 до 308 секунд (примерно на 62%) в контрольном прогоне без фоновой CPU-нагрузки.
Финальный прогон расширенного набора занял 590,8 секунды при параллельном стороннем
процессе аудита зависимостей. Linux release-QA по-прежнему запускает модули и
