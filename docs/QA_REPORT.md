...
Упало:                       0
Функциональных модулей:      55
Statements:                  18 002
Missed statements:           2 103
Statement coverage:          88,32%
Обязательный gate:           80%
Release-style прогон:        1 188,8 секунды, 55 изолированных процессов
Coverage-прогон:             проверенная базовая матрица + 96,2 секунды новых ветвей
```

Финальный release-style запуск завершён с 445 passed, 2 ожидаемыми Windows skip и без падений.
...
802 до 363,3 секунды (примерно на 55%) на предыдущем монолитном baseline. Текущий монолитный
прогон под параллельной системной нагрузкой превысил внешний лимит 600 секунд без test failure,
поэтому доказательным результатом принят изолированный прогон всех 55 модулей за 1 188,8 секунды.
